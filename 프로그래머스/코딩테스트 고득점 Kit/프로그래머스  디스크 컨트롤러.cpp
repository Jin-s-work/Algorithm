...
int solution(vector<vector<int>> jobs) {
    ...
    while(idx < jobs.size() || !pq.empty()){
        if(idx < jobs.size() && t >= jobs[idx][0]){//시작시간이 t보다 전에 있는 경우에만
            pq.push({jobs[idx][1], jobs[idx][0]}); // {도착시간, 시작시간}으로 넣음
            idx++;                                 // 했으므로 인덱스는 이동
            continue;
            // 뒤에를 생략하고 넘겨줌
        }
        ...
        // 위를 만족하지 않을 경우에는
        if(!pq.empty()){
            t += pq.top().first;            // 위에서 pq를 {도착시간, 시작시간}으로 해줌
            answer += t - pq.top().second;
            pq.pop();
            // t를 (도착 시간 - 시작 시간)을 더해준다.
        }
        else{       // 즉 pq는 비어있지만, idx<jobs.size()이고, t가 시작보다 더 클 경우
            t = jobs[idx][0];
            // 비어있을 경우에는 처음 시작한 위치로 넣어주고 시작한다.
        }
